...
//     * */
//
//    public boolean calcular(String termos_consulta[], AVLAdaptado indice, ArrayList<String> dataset){
...
//            for (ParOcorrenciasID par : indice.busca(termo).getPares()) { // Para cada par (descrição) associado ao termo será feito o cálculo da relevância
//                int id = (Integer) par.getIDProduto();
//                inserirDados(dataset.size(), indice.busca(termo).getPares().size());
//
...
//                    for (ParOcorrenciasID par : indice.busca(termo).getPares()) { // Para cada lista de cada termo
